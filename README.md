...
**I2C address:** `0x2B` (7-bit; write byte `0x56`, read byte `0x57`)

**Device ID:** `0x54` (value returned by the `DEV_ID` register — not the bus address)
...
Read-only. Capacitor voltage in volts, `1V` per LSB — no conversion needed on the master
side. The firmware applies the divider/ADC scaling (including an empirical calibration
factor) internally. The register saturates at `255`. Updated continuously via ADC interrupt.

---

### KICK_A / KICK_B Registers (`0x04`, `0x05`)

Write-only, 2 bytes, **LSB first**. Writing a non-zero 16-bit pulse duration in
microseconds fires the corresponding solenoid. The two bytes must be written in a single
I2C transaction.

The pulse is generated by a hardware one-shot with `16us` resolution: durations are rounded
**up** to the next multiple of `16us`, and values above `15984us` are clamped to that maximum.
...
Some faults also change `STATUS` as a side effect, visible to the master:

- `CHARGE_TIMEOUT` stops charging and clears `STATUS.CHARGE`.
- `DISCHARGE_STUCK` disables the discharge output and clears `STATUS.DISCHARGE`.
